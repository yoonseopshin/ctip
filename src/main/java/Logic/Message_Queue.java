    ...
    try {
      while (true) {
        ...
        objectInputStream = new ObjectInputStream(
            socket.getInputStream()); // Client 로부터 객체를 읽어오는 역활을 하는 객체를 생성
        ...
        msgQueue.offer(message); // 전송받은 메시지를 큐에 집어넣기
          if (message.getType() == 1) {
              System.out.print("재고요청메시지 수신됨\n");
          }
          if (message.getType() == 2) {
              System.out.print("재고응답메시지 수신됨\n");
          }
          if (message.getType() == 3) {
              System.out.print("위치요청메시지 수신됨\n");
          }
          if (message.getType() == 4) {
              System.out.print("위치응답메시지 수신됨\n");
          }
          if (message.getType() == 5) {
              System.out.print("인증번호메시지 수신됨\n");
          }
        printWriter.write("1");
    ...
    //서버로 접속하고 인풋스티림을 지정하는 부분
    {
      while (true) {
        ...
        ObjectOutputStream objectOutputStream = new ObjectOutputStream(
            socket.getOutputStream());//직렬화를 위한 객체 생성
